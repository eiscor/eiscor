...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine z_uprk_compress_slow(QZ,VECR,VECL,ID,N,K,Ain,Bin,P,Q,D1,&
     ...
     do jj = 1,(N-1)
        ...
        do row = jj,N-2 ! first row of the rotation
           ...
           if (col==0) then
              ...
              ! through A
              call z_uprkutri_rot3swap(.FALSE.,N,K,ii+1,K,D1,C1,B1,bulge,row+1)
              col = ii
           end if
        end do
...
end subroutine z_uprk_compress_slow
