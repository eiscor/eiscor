...
!
!  D1,D2           REAL(8) arrays of dimension (2*K*(N+1))
...
!!$  end if  
  ...
  if (QZ) then
     do ll = 1,k
        call z_upr1_factoridpspike(.TRUE.,N,N+1-ll,B(1:N,k+1-ll),&
             &D2(((ll-1)*2*(N+1)+1):(ll*2*(N+1))),&
        ...
        do row = jj,N-2 ! first row of the rotation
           ...
           if (col==0) then
              ...
              if (QZ) then
                 ...
                 bulge(3) = -bulge(3)
                 ! through B
                 call z_uprkfact_rot3throughalltri(.TRUE.,N,K,D2,C2,B2,bulge,row+1)
                 ! invert bulge
                 bulge(1) = bulge(1)
                 bulge(2) = -bulge(2) 
                 bulge(3) = -bulge(3)
