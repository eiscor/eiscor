...
subroutine z_upr1fact_singlestep(QZ,VEC,FUN,N,P,Q,D1,C1,B1,D2,C2,B2,M,V,W,ITCNT)
  ...
  !print*, N, shift, cmplx(0d0,1d0,kind=8)*(cmplx(1d0,0d0,kind=8)-shift)/(cmplx(1d0,0d0,kind=8)+shift)
  !if (N.EQ.2) then
  !   print*, Q(3), shift
  !end if
  ...
  if (N.LT.3) then
     ...
     call z_upr1fact_buildbulge(QZ,.FALSE.,tq,D1(1:4),C1(1:6),B1(1:6) &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (N.EQ.2) then
      ...
      ! merge from left
      call z_upr1fact_mergebulge(.TRUE.,N,.FALSE.,Q,D1(1:4),G1)
      ...
      ! merge from left
      call z_upr1fact_mergebulge(.TRUE.,N,.TRUE.,Q(1:3),D1(1:(2*N)),G2)
