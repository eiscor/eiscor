...
program example_z_uprkfact_randommatr
  ...
  call z_uprk_compress(.FALSE.,.TRUE.,.TRUE.,N,k,MA,MB,P,Q,D,C,B,D2,C2,B2,V,W,INFO)
  if (INFO.NE.0) then
...
!!$  end do
  ...
  do jj=1,N
     do ii=1,N
        if (output.AND.abs(W3(ii,jj)-CD(ii,jj)).GT.1d-14) then
...
!!$  end do
  ...
  call z_uprkfpen_qz(.TRUE.,.FALSE.,l_upr1fact_hess,N,k,&
       &P,Q,D,C,B,D2,C2,B2,N,V,W,ITS,INFO)
  if (INFO.NE.0) then
  ...
  do jj=1,N
     do ii=1,N
        if (output.AND.abs(W3(ii,jj)-CD(ii,jj))>1e-14) then
           print*, ii, jj, W3(ii,jj), CD(ii,jj), abs(W3(ii,jj)-CD(ii,jj))
        end if
        h = h + abs(W3(ii,jj)-CD(ii,jj))**2
     end do
