...
program example_z_uprkfact_randommatr
  ...
  if ((dd==2).AND.(k==2)) then
     REIGS(1) = cmplx( 5.850853916553567e-01, + 1.960620778250882e+00,kind=8)
     REIGS(2) = cmplx(-1.643275827808684e+00, + 9.249101310198671e-01,kind=8)
     REIGS(3) = cmplx(-1.665510376790535e-01, - 3.317227393069380e-01,kind=8)
     REIGS(4) = cmplx( 8.478546194285020e-01, - 5.774811996649782e-01,kind=8)
  end if
  ...
  if ((dd==4).AND.(k==1)) then
     ...
     REIGS(1) = cmplx( -4.506367401502382e-01, + 8.258089507059133e-01,kind=8)
     REIGS(2) = cmplx(  5.665606540229318e-01, + 5.700065546795512e-01,kind=8)
     REIGS(3) = cmplx( -2.314228515628051e-01, - 3.194639432989243e-01,kind=8)
     REIGS(4) = cmplx(  7.234246907448657e-01, - 2.718456363764743e+00,kind=8)
     do ii=1,N
        print*, MB(ii,1)
     end do
  end if
