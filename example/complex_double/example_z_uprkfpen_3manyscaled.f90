...
program example_z_uprkfpen_2manyscaled
  ...
  logical, parameter :: output=.FALSE., david = .FALSE.
...
!  end if
  ...
  do co2 = 14,14
     ...
     do co = 1,maxnumber
        ...
        MB = cmplx(0d0,0d0,kind=8)
        do ii=1,k
           MB(N-k+ii,ii)=cmplx(1d0,0d0,kind=8)
        end do
...
!!$
  ...
  write(8,*) "(",norma,",",sqrt(h),")%A",sqrt(h)/norma/norma

  if (sqrt(h)/norma/norma.GT.10*EISCOR_DBL_EPS) then
     print*, ITS
     sumit = 0
     do ii=1,N-1
        write(8,*) ii, ITS(ii)
  ...
  ! Rough estimate for the backward error on the second part of the
  ! algorithm, assuming it is much larger than the one in the first part.
  ! This is a lower bound, and since it actually _is_ larger, it is also
  ! probably good enough. 
  write(12,*) norma, sqrt(h) - afactbe
  ...
  write(9,*) "(",norma,",",sqrt(h),")%B",sqrt(h)/norma/norma

  ! Rough estimate for the backward error on the second part of the
  ! algorithm, assuming it is much larger than the one in the first part.
  ! This is a lower bound, and since it actually _is_ larger, it is also
  ! probably good enough. 
  write(13,*) normb, sqrt(h) - bfactbe
