...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! This program tests the subroutine z_rot4_vec4gen (generating rotations). 
...
program test_z_rot4_vec4gen
  ...
  ! check 0)
    ! all zeros
    AR = 0; AI = 0; BR = 0; BI = 0
    call z_rot4_vec4gen(AR,AI,BR,BI,CR,CI,SR,SI,NRM)
    if ((CR.NE.1d0).OR.(CI.NE.0d0).OR.(SR.NE.0d0).OR.(SI.NE.0d0).OR.(NRM.NE.0d0)) then
    ...
    if ((CR.NE.1d0).OR.(CI.NE.0d0).OR.(SR.NE.0d0).OR.(SI.NE.0d0).OR.(NRM.NE.inf)) then
      call u_test_failed(__LINE__)
    end if
    
    AR = 1d0; AI = inf; BR = 1d0; BI = 1d0
    call z_rot4_vec4gen(AR,AI,BR,BI,CR,CI,SR,SI,NRM)
    if ((CR.NE.0d0).OR.(CI.NE.1d0).OR.(SR.NE.0d0).OR.(SI.NE.0d0).OR.(NRM.NE.inf)) then
      call u_test_failed(__LINE__)
    end if
    
    AR = 1d0; AI = 1d0; BR = inf; BI = 1d0
    call z_rot4_vec4gen(AR,AI,BR,BI,CR,CI,SR,SI,NRM)
    if ((CR.NE.0d0).OR.(CI.NE.0d0).OR.(SR.NE.1d0).OR.(SI.NE.0d0).OR.(NRM.NE.inf)) then
      call u_test_failed(__LINE__)
    end if
    
    AR = 1d0; AI = 1d0; BR = 1d0; BI = inf
    call z_rot4_vec4gen(AR,AI,BR,BI,CR,CI,SR,SI,NRM)
    if ((CR.NE.0d0).OR.(CI.NE.0d0).OR.(SR.NE.0d0).OR.(SI.NE.1d0).OR.(NRM.NE.inf)) then
      call u_test_failed(__LINE__)
    end if
    ...
    call z_rot4_vec4gen(AR,AI,BR,BI,CR,CI,SR,SI,NRM)
    if ((CR.NE.0d0).OR.(CI.NE.-1d0).OR.(SR.NE.0d0).OR.(SI.NE.0d0).OR.(NRM.NE.inf)) then
      print*, AR, AI, BR, BI, CR, CI, SR, SI, NRM
      call u_test_failed(__LINE__)
    end if
    ...
    call z_rot4_vec4gen(AR,AI,BR,BI,CR,CI,SR,SI,NRM)
    if ((CR.EQ.CR).OR.(CI.EQ.CI).OR.(SR.EQ.SR).OR.(SI.EQ.SI).OR.(NRM.EQ.NRM)) then
      call u_test_failed(__LINE__)
    end if
  ...
  if (abs(SR-sqrt(2d0)/2d0)>tol) then
    call u_test_failed(__LINE__)
  end if
  if (abs(SI)>tol) then
     print*, AR, AI, BR, BI, CR, CI, SR, SI, NRM
  ...
  if (abs(SR-1d0/sqrt(3d0))>tol) then
    call u_test_failed(__LINE__)
  end if
  if (abs(SI)>tol) then
    call u_test_failed(__LINE__)
 ...
 if (abs(NRM-1d0)>tol) then
  ...
  ! check results
  if (abs(CR-sqrt(9d0/30d0))>tol) then
     print*, AR, AI, BR, BI, CR, CI, SR, SI, NRM
     call u_test_failed(__LINE__)
  end if
  if (abs(CI-sqrt(4d0/30d0))>tol) then
     print*, AR, AI, BR, BI, CR, CI, SR, SI, NRM
     call u_test_failed(__LINE__)
  end if
  if (abs(SR-sqrt(1d0/30d0))>tol) then
     print*, AR, AI, BR, BI, CR, CI, SR, SI, NRM
     call u_test_failed(__LINE__)
  end if 
  if (abs(SI+sqrt(16d0/30d0))>tol) then
     print*, AR, AI, BR, BI, CR, CI, SR, SI, NRM
     call u_test_failed(__LINE__)
  end if
...
end program test_z_rot4_vec4gen
